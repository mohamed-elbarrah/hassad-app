...
export default function PortalDeliverablesPage() {
  ...
  const { data: selectedProject } = useGetProjectReviewDetailQuery(
    selectedId!,
    { skip: !selectedId, pollingInterval: PORTAL_POLLING_INTERVAL_MS },
  );
  useEffect(() => {
          ...
          <CardContent className="pt-6">
            <Empty>
              <EmptyHeader>
                ...
                <EmptyDescription>
                  يرجى التواصل مع الإدارة لربط حسابك بملف العميل.
                </EmptyDescription>
              </EmptyHeader>
            </Empty>
          </CardContent>
    ...
    <main dir="rtl" className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <Popover>
          ...
          >
            <div>
              <p className="font-medium">حالة التسليم</p>
              <p className="text-sm text-muted-foreground">اختر حالة واحدة.</p>
            </div>
            <Separator />
            {options.map((status) => {
              const id = `deliverable-status-${status}`;
            ...
            <TableBody>
              {filtered.map((project) => (
                <TableRow key={project.id}>
                  ...
                  <TableCell>{project.manager?.name ?? "بدون مدير"}</TableCell>
                  <TableCell>{project.deliverableCount}</TableCell>
                  <TableCell>
                    <DomainStatusPill
                      domain="project"
                  ...
                  <TableCell>
                    <Button size="sm" onClick={() => setSelectedId(project.id)}>
                      <Eye />
                      مراجعة
                    </Button>
                  </TableCell>
          ...
          <CardContent className="pt-6">
            <Empty>
              <EmptyHeader>
                ...
                <EmptyTitle>لا توجد مشاريع بانتظار المراجعة</EmptyTitle>
                <EmptyDescription>
                  ستظهر هنا المشاريع عندما يقدمها فريقك للمراجعة.
                </EmptyDescription>
              </EmptyHeader>
            </Empty>
          </CardContent>
      ...
      <ReviewModal
        ...
        onActionComplete={() => {
          refetch();
          toast.success("تم تحديث قائمة المراجعة");
          setSelectedId(null);
        }}
      />
    </main>
