...
import type { AdminRequestItem } from "@/features/admin/adminRequestsApi";
  ...
  const metrics = useMemo(
    () => ({
      ...
      waiting: requests.filter(
        (i) =>
          i.status === RequestStatus.SUBMITTED ||
          i.status === RequestStatus.QUALIFYING,
      ).length,
    ...
    <div dir="rtl" className="flex flex-col gap-6   ">
      ...
      <Card>
        <CardHeader className="gap-2">
          ...
          <CardDescription>اضغط على السطر لفتح تفاصيل الطلب.</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_220px] max-w-3xl">
            ...
            >
              ...
              <SelectContent>
                ...
                {(Object.values(RequestStatus) as RequestStatus[]).map(
                  (value) => (
                    <SelectItem key={value} value={value}>
                      {REQUEST_STATUS_AR[value]}
                    </SelectItem>
                  ),
                )}
              </SelectContent>
            ...
            <Table>
              ...
              <TableBody>
                {requests.length === 0 ? (
                  ...
                  requests.map((request: AdminRequestItem) => (
                    <TableRow key={request.id}>
                      ...
                      <TableCell>
                        {formatNumber(request.servicesCount)}
                      </TableCell>
                      <TableCell>{formatNumber(request.ageDays)} يوم</TableCell>
