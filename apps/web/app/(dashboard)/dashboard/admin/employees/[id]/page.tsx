...
function LoadingState() {
  return (
    <div className="flex flex-col gap-6" dir="rtl">
      <PageHeader
        title="تفاصيل الموظف"
        description="جارٍ تحميل بيانات الموظف."
        icon={Users}
      />
...
export default function EmployeeDetailPage({
  ...
  const [activate] = useActivateAdminUserMutation();
  const [deactivate] = useDeactivateAdminUserMutation();
        ...
        <Card>
          <CardContent className="p-8">
            <Empty>
              ...
              <EmptyHeader>
                <EmptyTitle>تعذر تحميل الموظف</EmptyTitle>
  ...
  const roleLabel =
    USER_ROLE_AR[employee.role as keyof typeof USER_ROLE_AR] ?? employee.role;
  const departmentLabel = employee.department
    ? (TASK_DEPARTMENT_AR[
        employee.department as keyof typeof TASK_DEPARTMENT_AR
      ] ?? employee.department)
    : "غير محدد";
  ...
  return (
    <div className="flex flex-col gap-6" dir="rtl">
        ...
        actions={
          <>
            <AccountLifecycleActions
              ...
              isSuspended={Boolean(employee.suspendedAt)}
              onAction={runLifecycleAction}
            />
            {employee.isActive ? (
              <Button variant="outline" asChild>
                <Link
                  href={`/dashboard/admin/chat?userId=${encodeURIComponent(employee.id)}`}
                >
            ...
            <Card key={metric.key}>
              <CardContent className="flex min-h-28 flex-col justify-between gap-3 p-5">
                <span className="text-sm text-muted-foreground">
                  {adminEmployeeMetricLabel(metric.key)}
                </span>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <PerformanceSection
          sectionCode={data.performance.sectionCode}
          metrics={data.performance.metrics}
        />
        <ProfileDetails employee={employee} />
      </div>

      {data.work.campaigns.length > 0 ? (
        <WorkTable
          title="الحملات التي أنشأها الموظف"
          description="الحملات المرتبطة بهذا الموظف بصفته المنشئ."
          headers={["الحملة", "العميل", "المنصة", "الحالة"]}
          rows={data.work.campaigns.map((campaign) => [
            campaign.name,
            campaign.clientName,
            CAMPAIGN_PLATFORM_AR[
              campaign.platform as keyof typeof CAMPAIGN_PLATFORM_AR
            ] ?? campaign.platform,
            CAMPAIGN_STATUS_AR[
              campaign.status as keyof typeof CAMPAIGN_STATUS_AR
            ] ?? campaign.status,
          ])}
        />
      ) : null}
      {data.work.projects.length > 0 ? (
        <WorkTable
          ...
          description="المشاريع المرتبطة بهذا الموظف."
          headers={["المشروع", "العميل", "الحالة"]}
          rows={data.work.projects.map((project) => [
            project.name,
            project.clientName,
            PROJECT_STATUS_AR[
      ...
      {data.work.tasks.length > 0 ? (
        <WorkTable
          ...
          description="المهام المرتبطة بهذا الموظف."
          headers={["المهمة", "المشروع", "الحالة"]}
          rows={data.work.tasks.map((task) => [
            task.title,
            task.projectName,
      ...
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" asChild>
          <Link href={`/dashboard/admin/employees/${employee.id}/activity`}>
            <CheckSquare data-icon="inline-start" />
            سجل النشاط
          </Link>
        </Button>
        <Button variant="outline" asChild>
          <Link href={`/dashboard/admin/employees/${employee.id}/permissions`}>
    ...
    <Card>
      <CardContent className="flex flex-col gap-6 p-6 sm:flex-row sm:items-start">
        <Avatar className="size-20">
          <AvatarImage src={employee.avatarUrl ?? undefined} alt="" />
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 text-sm text-muted-foreground sm:flex-row sm:flex-wrap sm:gap-5">
            <span className="inline-flex items-center gap-2">
              <Mail aria-hidden="true" />
              {employee.email}
            </span>
            {employee.phoneWhatsapp ? (
              <span>{employee.phoneWhatsapp}</span>
            ) : null}
          </div>
        </div>
      </CardContent>
  ...
  return (
    <Card>
      <CardHeader className="gap-2">
        ...
        <CardDescription>بيانات الحساب والحالة الأمنية للموظف.</CardDescription>
      </CardHeader>
      <CardContent className="grid gap-x-8 gap-y-5 sm:grid-cols-2">
        ...
        <DetailItem
          icon={Users}
          label="رقم التواصل"
          value={employee.phoneWhatsapp ?? "غير محدد"}
        />
        <DetailItem
          icon={CalendarDays}
          label="آخر دخول"
        ...
        <DetailItem
          ...
          value={employee.twoFactorEnabled ? "مفعلة" : "غير مفعلة"}
        />
        <DetailItem
          icon={CalendarDays}
          label="تاريخ الإنشاء"
...
function PerformanceSection({
  ...
  return (
    <Card>
      <CardHeader className="gap-2">
        <CardTitle>{adminEmployeeSectionTitle(sectionCode)}</CardTitle>
        <CardDescription>
          المؤشرات التشغيلية الخاصة بدور هذا الموظف.
        </CardDescription>
      </CardHeader>
