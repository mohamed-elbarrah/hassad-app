...
import { Settings, Save, RotateCcw, Building2, FileText, Palette, Shield, Brain, Bell } from "lucide-react";
...
export default function AdminSettingsPage() {
    ...
    try {
      ...
      for (const defs of Object.values(FIELD_DEFS)) {
        for (const def of defs) {
          ...
          } else if (["suspend_on_overdue", "brand_white_label", "security_2fa_enforced",
            "ai_gemini_enabled", "ai_auto_lead_scoring", "ai_auto_task_suggestions",
            "notif_email_enabled", "notif_sms_enabled", "notif_in_app_enabled"].includes(def.key)) {
            val = val === "true" || val === "1";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {defs.map((def) => (
          <div key={def.key} className="flex flex-col gap-1.5">
            ...
            {def.type === "select" ? (
              <select value={form[def.key] ?? ""} onChange={(e) => handleChange(def.key, e.target.value)}
              ...
              <FormInputControl type={def.type} value={form[def.key] ?? ""}
                onChange={(e) => handleChange(def.key, e.target.value)} placeholder={def.placeholder} />
            )}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="flex flex-col gap-6" dir="rtl">
      <PageIntro title="إعدادات المنصة" description="تكوين جميع إعدادات المنصة" icon={Settings}
        actions={isDirty ? (
          <div className="flex gap-2">
            <ActionButton variant="outline" size="md" onClick={handleReset} disabled={isSaving}>
              <RotateCcw className="size-4 ml-1" />استعادة
            </ActionButton>
            <ActionButton size="md" onClick={handleSave} disabled={isSaving}>
              <Save className="size-4 ml-1" />{isSaving ? "جارٍ الحفظ..." : "حفظ"}
            </ActionButton>
          </div>
        ) : undefined}
      />
              ...
              {TABS.map((tab) => {
                ...
                return (
                  <TabsTrigger key={tab.value} value={tab.value} className="whitespace-nowrap">
                    <Icon className="size-4 ml-1" />{tab.label}
                  </TabsTrigger>
                );
              })}
            </TabsList>
