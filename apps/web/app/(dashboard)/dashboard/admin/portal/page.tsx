...
export default function AdminPortalPage() {
  ...
  const { data: clientsData, isLoading } = useGetPortalClientsQuery({
  ...
  return (
    <div className="flex flex-col gap-6" dir="rtl">
      <PageIntro
        title="بوابة العملاء"
        description="نظرة عامة على نشاط العملاء في البوابة"
        icon={Globe}
      />
      ...
      {overview && (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            icon={Users}
            trend="up"
            trendValue={`${overview.idleClients} خامل`}
          />
          <StatCard
          ...
          <StatCard
            ...
            value={overview.pendingRevisions}
            icon={FileText}
            variant={overview.pendingRevisions > 0 ? "warning" : "default"}
          />
          <StatCard
            title="نماذج غير مكتملة"
            value={overview.unsubmittedIntakeForms}
            icon={AlertTriangle}
            variant={
              overview.unsubmittedIntakeForms > 0 ? "warning" : "default"
            }
          />
        </div>
      )}
      ...
      <DataTable
        columns={[
          ...
          { id: "lastLogin", label: "آخر دخول", align: "left" },
          { id: "intake", label: "النموذج" },
        ...
        renderRow={(c: PortalClientRow) => (
          <tr key={c.id} className="border-b border-portal-divider">
            ...
            <td className="px-5 py-4">
              {c.hasPortalAccess ? (
                <Pill tone="success">مفعلة</Pill>
              ) : (
                <Pill tone="neutral">غير مفعلة</Pill>
              )}
            </td>
            <td
              ...
              dir="ltr"
            >
              {c.lastLoginAt?.slice(0, 10) ?? "—"}
            </td>
            <td className="px-5 py-4">
              {c.intakeCompleted ? (
                <CheckCircle className="size-4 text-success-500" />
              ) : (
                <Clock className="size-4 text-warning-500" />
              )}
            </td>
            <td className="px-5 py-4">
              {c.pendingApprovalsCount > 0 ? (
                <Pill tone="warning">{c.pendingApprovalsCount}</Pill>
              ) : (
      ...
      <Dialog
        ...
        description={
          newToken
            ? "تم إنشاء الرمز الجديد. يمكنك نسخه الآن."
            : `سيتم إنشاء رمز جديد للعميل ${tokenClient?.companyName}. الرمز القديم سينتهي فوراً.`
        }
        footer={
          <div className="flex gap-2 justify-end">
            ...
            {!newToken && (
              <ActionButton onClick={handleRegenerateToken}>تأكيد</ActionButton>
            )}
          </div>
        }
      >
        {newToken && (
          <div className="space-y-2">
            ...
            <div
              className="rounded-xl border border-portal-divider bg-badge-gray-bg p-3 font-mono text-sm break-all"
              dir="ltr"
            >
              {newToken}
            </div>
