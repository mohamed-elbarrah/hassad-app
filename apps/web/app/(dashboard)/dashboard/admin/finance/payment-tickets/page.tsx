...
import { TICKET_STATUS_AR } from "@hassad/shared";

export default function AdminPaymentTicketsPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const { data, isLoading, isError } = useGetPaymentTicketsQuery({
    page,
    limit: 20,
  });
  ...
  const filtered = search
    ? tickets.filter(
        (t: any) =>
          t.description?.includes(search) || t.invoiceId?.includes(search),
      )
    : tickets;
  ...
  return (
    <div className="flex flex-col gap-6" dir="rtl">
      <PageIntro
        title="تذاكر الدفع"
        description="إدارة تذاكر الدفع والمشكلات المالية"
        icon={Ticket}
      />
