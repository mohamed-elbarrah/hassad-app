  ...
  return (
    <div className="flex flex-col gap-6" dir="rtl">
      <PageIntro
        ...
        description="حالة النظام والخدمات الخارجية"
        icon={Monitor}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatCard
          title="وقت التشغيل"
          value={data?.uptime ? `${Math.floor(data.uptime / 3600)} ساعة` : "—"}
          icon={Clock}
        />
        <StatCard
          title="الترحيلات المعلقة"
          value={`${data?.pendingMigrations ?? 0}`}
          icon={Database}
          extra={
          ...
          {(data?.externalServices ?? []).map((s: any) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`size-3 rounded-full ${s.status === "UP" ? "bg-success-500" : s.status === "DEGRADED" ? "bg-warning-500" : "bg-danger-500"}`}
                />
                <div>
                  <p className="text-sm font-medium">{s.serviceName}</p>
                  <p className="text-xs text-portal-note-text">
                    زمن الاستجابة: {s.responseTime}ms
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
      ...
      <SurfaceCard title="الذاكرة">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <StatCard
            title=" RSS"
            value={
              data?.memoryUsage?.rss
                ? `${(data.memoryUsage.rss / 1024 / 1024).toFixed(0)} MB`
                : "—"
            }
            icon={MemoryStick}
          />
          <StatCard
            title="Heap Total"
            value={
              data?.memoryUsage?.heapTotal
                ? `${(data.memoryUsage.heapTotal / 1024 / 1024).toFixed(0)} MB`
                : "—"
            }
            icon={MemoryStick}
          />
          <StatCard
            ...
            value={
              data?.memoryUsage?.heapUsed
                ? `${(data.memoryUsage.heapUsed / 1024 / 1024).toFixed(0)} MB`
                : "—"
            }
            icon={MemoryStick}
          />
