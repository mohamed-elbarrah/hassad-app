            ...
            <SurfaceCard className="border-2 border-secondary-500/10">
              <div className="bg-secondary-500/5 px-6 py-4 border-b border-secondary-500/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-lg text-natural-100">
                      Stripe
                    </h3>
                    <p className="text-sm text-portal-note-text">
                      قبول المدفوعات عبر بطاقات الائتمان، مدى، و Apple Pay
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 bg-natural-0 px-3 py-1.5 rounded-full border border-portal-card-border shadow-sm">
                  <Checkbox
                    ...
                    onCheckedChange={(val) =>
                      setStripeConfig((prev) => ({ ...prev, isActive: !!val }))
                    }
                  />
                  <Label
                    htmlFor="stripe-active"
                    className="cursor-pointer font-medium text-natural-100"
                  >
                    تفعيل البوابة
                  </Label>
                </div>
              ...
              <div className="p-6 space-y-6">
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label className="text-sm font-semibold text-natural-100">
                      Publishable Key
                    </Label>
                    <FormInputControl
                    ...
                    <p className="text-[10px] text-portal-note-text">
                      يستخدم في الواجهة الأمامية لتشفير بيانات البطاقة
                    </p>
                  </div>
                  <div className="space-y-2">
                    <Label className="text-sm font-semibold text-natural-100">
                    ...
                    />
                    <p className="text-[10px] text-portal-note-text">
                      مفتاح سري للاستخدام في الخادم فقط (يتم تشفيره في قاعدة
                      البيانات)
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-[10px] text-portal-note-text">
                    يستخدم للتحقق من صحة التنبيهات القادمة من Stripe
                  </p>
                </div>
                ...
                <div className="bg-alert-50 border border-alert-100 rounded-2xl p-4 flex gap-3 items-start">
                  ...
                  <div className="text-xs text-alert-800 leading-relaxed">
                    <p className="font-bold mb-1">
                      تأكد من إعداد Webhook في لوحة تحكم Stripe:
                    </p>
                    <p>
                      رابط الاستقبال:{" "}
                      <code className="bg-alert-100 px-1 rounded">
                        https://api.hassad.sa/v1/webhooks/stripe
                      </code>
                    </p>
                    <p>
                      الأحداث المطلوبة:{" "}
          ...
          <TabsContent value="manual" className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-portal-note-text">
                  هذه الحسابات ستظهر للعملاء عند اختيار الدفع بالتحويل البنكي
                </p>
              </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {bankAccounts?.map((account) => (
                <SurfaceCard
                  key={account.id}
                  className="relative group hover:border-secondary-500/50 transition-colors"
                >
                  <div className="pb-2">
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="text-[10px] text-portal-note-text uppercase tracking-wider mb-1">
                        اسم الحساب
                      </div>
                      <div className="text-sm font-semibold text-natural-100">
                        {account.accountName}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="text-sm font-mono bg-portal-bg p-2 rounded-xl border text-center select-all text-natural-100">
                        {account.iban}
                      </div>
                    </div>
                    <div className="flex justify-between items-center pt-2">
                      <div className="flex items-center gap-1.5">
                        <div
                          className={`w-2 h-2 rounded-full ${account.isActive ? "bg-success-500" : "bg-danger-500"}`}
                        />
    ...
    >
      <div className="space-y-4">
        <div className="grid gap-2">
          <Label className="text-natural-100">
            اسم البنك <span className="text-danger-500">*</span>
          </Label>
          <FormInputControl
            value={data.bankName}
        ...
        <div className="grid gap-2">
          <Label className="text-natural-100">
            اسم صاحب الحساب <span className="text-danger-500">*</span>
          </Label>
          <FormInputControl
        ...
        <div className="grid gap-2">
          <Label className="text-natural-100">
            رقم الآيبان (IBAN) <span className="text-danger-500">*</span>
          </Label>
          <FormInputControl
            value={data.iban}
