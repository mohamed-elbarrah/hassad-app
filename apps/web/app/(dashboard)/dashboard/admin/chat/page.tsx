...
export default function AdminChatPage() {
  ...
  const [selectedConv, setSelectedConv] = useState<ConversationRow | null>(
    null,
  );
  const [showMessages, setShowMessages] = useState(false);

  const debouncedSearch = useDebounce(searchInput, 400);
  const { data, isLoading, isError } = useGetAdminConversationsQuery({
  ...
  const handleHide = async (id: string) => {
    try {
      await hideConv(id).unwrap();
      toast.success("تم إخفاء المحادثة");
    } catch {
      toast.error("فشل");
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6" dir="rtl">
      <PageIntro
        title="المحادثات"
        description={`إجمالي ${data?.total ?? 0} محادثة`}
        icon={MessageSquare}
      />
      ...
      <DataTable
        columns={[
          ...
          { id: "lastMessage", label: "آخر رسالة" },
          { id: "messages", label: "الرسائل" },
          { id: "status", label: "الحالة" },
          { id: "lastActive", label: "آخر نشاط", align: "left" },
        ...
        emptyState={{
          ...
          hint: "لم يتم إنشاء أي محادثات بعد",
        }}
        renderRow={(c: ConversationRow) => (
          <tr key={c.id} className="border-b border-portal-divider">
            <td className="px-5 py-4 text-sm text-natural-100">
              {c.participants.map((p) => p.name).join("، ")}
            </td>
            <td className="px-5 py-4 text-sm text-portal-note-text max-w-[200px] truncate">
            ...
            <td className="px-5 py-4">
              {c.isStale ? (
                <Pill tone="warning">خاملة</Pill>
              ) : (
                <Pill tone="success">نشطة</Pill>
              )}
            </td>
            <td
      ...
      <Dialog
        ...
        footer={
          <ActionButton onClick={() => setShowMessages(false)}>
            إغلاق
          </ActionButton>
        }
      >
          ...
          {(messagesData?.items ?? []).map((m: any) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-portal-note-text" dir="ltr">
              ...
              {m.attachments?.length > 0 && (
                <div className="mt-2 flex gap-2">
                  {m.attachments.map((a: any) => (
                    <span key={a.id} className="text-xs text-secondary-500">
                      {a.fileName}
                    </span>
                  ))}
                </div>
