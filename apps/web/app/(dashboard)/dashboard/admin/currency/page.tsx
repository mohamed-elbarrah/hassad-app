...
export default function CurrencySettingsPage() {
  ...
  const openEdit = (currency: any) => {
    setForm({
      ...
      svgKey: currency.svgKey ?? "",
      svgWidth: currency.svgWidth ?? 24,
      svgHeight: currency.svgHeight ?? 24,
  ...
  const validate = (): boolean => {
    ...
    if (!form.code.trim()) next.code = "رمز العملة مطلوب";
      ...
      next.code = "يجب أن يكون 3 حروف (ISO)";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.symbolType !== "TEXT") {
      ...
      payload.svgWidth = form.svgWidth;
      payload.svgHeight = form.svgHeight;
    }
    try {
      if (editingId) {
  ...
  const previewSvgUrl =
    form.symbolType === "SVG_URL" && form.svgKey.trim()
      ? form.svgKey.trim()
      : null;
      ...
      <DataTable
        ...
        renderRow={(c: any) => (
          <tr key={c.id} className="border-b-[1.5px] border-portal-divider">
            ...
            <td className="px-5 py-4 text-base font-mono text-natural-100 text-left">
              {c.exchangeRate}
            </td>
            <td className="px-5 py-4">
              <StatusBadge
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {form.symbolType === "SVG_URL" ? (
                  <FormInputControl
                    ...
                    value={form.svgKey}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, svgKey: e.target.value }))
                    }
                    onBlur={() =>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className={cn(
                      "flex w-full rounded-xl border border-neutral-200 bg-white px-3 py-2 text-sm shadow-sm placeholder:text-neutral-200 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-secondary-500/20 min-h-[80px] resize-y font-mono ltr-dir",
                      errors.svgKey &&
                        "border-danger-500 focus-visible:ring-danger-500",
                    )}
                  />
          ...
          {form.isDefault && (
            <div className="rounded-2xl bg-alert-50 border border-alert-200 p-4 flex items-start gap-2 text-sm">
              ...
              <p className="text-alert-800">
                سيتم تعيين هذه العملة كافتراضية لجميع العملاء في البوابة
                واستبدال العملة الافتراضية الحالية تلقائيًا.
              </p>
            </div>
          )}
          ...
          <div className="rounded-2xl border border-dashed border-portal-card-border bg-portal-bg p-4 flex flex-col items-center gap-2">
            <span className="text-xs text-portal-note-text mb-1">
              معاينة العرض
            </span>
