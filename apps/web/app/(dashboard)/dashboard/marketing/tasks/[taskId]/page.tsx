...
export default function MarketingTaskDetailPage() {
  ...
  const { data: campaigns = [], isLoading: isCampaignsLoading } =
    useGetCampaignsByTaskQuery(taskId);
  ...
  const totalRoas =
    aggregated.spend > 0
      ? (aggregated.rev / aggregated.spend).toFixed(2)
      : "0.00";
    ...
    <div className="flex flex-col gap-6 pb-10" dir="rtl">
      ...
      <div className="flex flex-col gap-4 border-b pb-6">
        <Link
          ...
          className="text-sm text-neutral-300 flex items-center gap-1 hover:text-secondary-500 transition-colors w-fit"
        >
      ...
      <Tabs defaultValue="campaigns" className="space-y-6">
        ...
        <TabsContent value="campaigns" className="space-y-6">
          ...
          {campaigns.length === 0 ? (
            <SurfaceCard
              ...
              contentClassName="flex flex-col items-center justify-center text-center py-12"
            >
              <Layout className="w-12 h-12 text-neutral-300/40 mb-4" />
              <p className="text-neutral-300 font-medium">
                لا توجد حملات مرتبطة بهذه المهمة بعد.
              </p>
              <ActionButton
                variant="outline"
                className="mt-4"
        ...
        <TabsContent value="overview">
          <SurfaceCard
            ...
            title="وصف المهمة والمتطلبات"
            contentClassName="prose prose-sm max-w-none text-neutral-300 leading-relaxed"
          >
            <p>{task.description}</p>
            <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-6">
              <div>
                <h5 className="text-[10px] uppercase font-bold text-neutral-300 mb-1">
                  تاريخ الاستحقاق
                </h5>
        ...
        <TabsContent value="performance">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <PerformanceMetricCard
              ...
              value={aggregated.conv.toString()}
...
function CampaignCard({
  ...
  return (
    <SurfaceCard
      className={`group relative shadow-sm border-neutral-50/60 hover:border-secondary-500/40 transition-all ${campaign.needsOptimization ? "border-danger-200 bg-danger-50/10" : ""}`}
      contentClassName="p-5"
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <Pill tone="neutral" className="text-[10px] font-bold">
            {campaign.platform}
          </Pill>
          {campaign.needsOptimization && (
            <Pill tone="danger" className="text-[9px] animate-pulse">
              يحتاج تحسين
            </Pill>
          )}
        </div>
      ...
      <h4 className="font-bold text-lg mb-4 group-hover:text-secondary-500 transition-colors">
        {campaign.name}
      </h4>
      ...
      <div className="grid grid-cols-3 gap-y-4 gap-x-2 mb-6 border-y py-4 border-neutral-50/30">
        <div>
          ...
          <p className="font-bold text-sm text-natural-100">
        ...
        <div>
          ...
          <p
            className={`font-bold text-sm ${parseFloat(metrics.roas) >= 2 ? "text-success-600" : "text-danger-600"}`}
          >
            {metrics.roas}x
          </p>
        </div>
