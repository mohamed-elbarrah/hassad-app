...
export default function PMTaskDetailPage({ params }: TaskDetailPageProps) {
  const { id } = use(params);
  ...
  const taskWithRelations = task as
    | (typeof task & {
  ...
  if (isError || !task || !taskWithRelations) {
    return (
  ...
  const backHref = taskWithRelations.project
    ? `/dashboard/pm/projects/${taskWithRelations.project.id}`
