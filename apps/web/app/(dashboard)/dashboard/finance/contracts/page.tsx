  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-neutral-300">متابعة تحصيل الدفعات مقارنة بالقيمة الإجمالية للعقود.</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-4">
        <SurfaceCard className="border-none shadow-sm" contentClassName="pt-0">
          <div>
            ...
            <h3 className="text-2xl font-bold mt-1">{totalValue.toLocaleString()} ر.س</h3>
          </div>
        ...
        <SurfaceCard className="border-none shadow-sm" contentClassName="pt-0">
          <div>
            ...
            <h3 className="text-2xl font-bold text-success-600 mt-1">{totalPaid.toLocaleString()} ر.س</h3>
          </div>
          <div className="mt-3 space-y-1">
            <ProgressBar value={averageCollectionRate} size="sm" />
            <p className="text-[10px] text-neutral-300">{averageCollectionRate.toFixed(1)}% من إجمالي القيمة</p>
          </div>
        </SurfaceCard>
        <SurfaceCard className="border-none shadow-sm" contentClassName="pt-0">
          <div>
            <p className="text-portal-note-text text-sm">المبالغ المتبقية</p>
            <h3 className="text-2xl font-bold text-danger-600 mt-1">{totalRemaining.toLocaleString()} ر.س</h3>
          </div>
          <div className="flex items-center text-xs text-neutral-300 mt-3">
      ...
      <div className="rounded-xl border border-portal-card-border bg-natural-0 shadow-sm">
        <div className="px-5 py-4 border-b border-portal-divider">
          <div className="relative max-w-sm">
            ...
            <FormInputControl placeholder="البحث عن عقد أو عميل..." className="pr-10" />
          </div>
        </div>
        <div className="p-5">
          <Table>
            ...
            <TableBody>
              {contracts.map((contract) => (
                <TableRow key={contract.id}>
                  <TableCell className="font-medium">
                    ...
                    <div className="text-[10px] text-neutral-300 font-mono">{contract.id.substring(0, 8)}...</div>
                  </TableCell>
                  ...
                  <TableCell className="text-success-600 font-medium">{contract.paid.toLocaleString()} ر.س</TableCell>
                  <TableCell className="text-danger-600 font-medium">{contract.remaining.toLocaleString()} ر.س</TableCell>
                  <TableCell className="w-[150px]">
                    <div className="space-y-1">
                      <ProgressBar value={contract.collectionRate} size="sm" />
                      <span className="text-[10px] text-neutral-300">{contract.collectionRate.toFixed(1)}%</span>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-left">
                    <Link href={`/dashboard/finance/contracts/${contract.id}`}>
                      <ActionButton variant="ghost" size="sm">التفاصيل</ActionButton>
                    </Link>
                  </TableCell>
                </TableRow>
              ))}
              {contracts.length === 0 && (
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-10 text-neutral-300">لا توجد عقود مسجلة.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
