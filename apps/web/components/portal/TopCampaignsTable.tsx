...
export function TopCampaignsTable({ campaigns }: TopCampaignsTableProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-1 text-[10px] text-muted-foreground px-2 py-1 bg-gray-50 rounded-lg">
        <button
          className={`flex-1 text-center py-1 rounded ${sortKey === "conversions" ? "bg-white shadow-sm text-foreground font-medium" : ""}`}
          onClick={() => handleSort("conversions")}
        >
          التحويلات {sortKey === "conversions" ? (sortAsc ? "↑" : "↓") : ""}
        </button>
        <button
          className={`flex-1 text-center py-1 rounded ${sortKey === "conversionRate" ? "bg-white shadow-sm text-foreground font-medium" : ""}`}
          onClick={() => handleSort("conversionRate")}
        >
          CTR {sortKey === "conversionRate" ? (sortAsc ? "↑" : "↓") : ""}
        </button>
        <button
          className={`flex-1 text-center py-1 rounded ${sortKey === "name" ? "bg-white shadow-sm text-foreground font-medium" : ""}`}
