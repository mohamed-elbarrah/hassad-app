...
export function ProgressBar({ value, max = 100, variant = "default", size = "md", showLabel = false, className }: ProgressBarProps) {
  ...
  return (
    <div className={cn("w-full", className)}>
      <div className={cn("w-full rounded-full bg-neutral-50 overflow-hidden", size === "sm" ? "h-1.5" : "h-2.5")}>
        <div
          className={cn("h-full rounded-full transition-all duration-300", variantFill[variant])}
          style={{ width: `${pct}%` }}
        />
      </div>
      {showLabel && <span className="text-xs text-neutral-300 mt-1">{pct.toFixed(0)}%</span>}
    </div>
  );
}
