...
async function main() {
  ...
  try {
    console.log(
      "Starting backfill: create projects for SIGNED contracts without projects",
    );
    ...
    for (const contract of contracts) {
      ...
      try {
        ...
        let projectManagerId =
          preferredManagers.find((c) => c.id === contract.client.accountManager)
            ?.id ??
        ...
        if (!projectManagerId) {
          ...
          if (!fallbackPm) {
            throw new Error(
              "No active PM account available to assign project manager",
            );
          }
          fallbackUsed = true;
        ...
        if (project) {
          createdCount += 1;
          console.log(
            `Created project ${project.id} for contract ${contract.id}`,
          );
        } else {
          console.log(
            `Skipped contract ${contract.id}, project already exists`,
          );
        }
    ...
    console.log(
      `Backfill complete. Projects created: ${createdCount}. Errors: ${errors.length}`,
    );
    if (errors.length > 0) {
      console.error(
        "Errors encountered during backfill. Aborting with non-zero exit code.",
      );
