...
describe("Workflow: Contract Branches", () => {
  test("Cancel contract after creation (SENT → CANCELLED)", async () => {
    ...
    const cancellationHistory = await db.contractStatusHistory.findFirst({
      where: { contractId: contract.id, toStatus: "CANCELLED" },
      select: { fromStatus: true, toStatus: true, changedBy: true },
    });
    expect(cancellationHistory).toMatchObject({
      fromStatus: "SENT",
      toStatus: "CANCELLED",
    });
