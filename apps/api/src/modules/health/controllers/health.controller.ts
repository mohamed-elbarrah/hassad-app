...
export class HealthController {
  ...
  async check() {
    ...
    externalResults.forEach((res, index) => {
      ...
      if (res.status === 'fulfilled') {
        ...
        const errorDetail = { status: 'down' as const, message: res.reason?.message || `${key} check failed` };
    ...
    // Save to history
    await this.healthPersistence.saveHealthCheck(result, Date.now() - startTime);
  ...
  async getHistory(@Query() query: HealthHistoryQueryDto) {
    return this.healthPersistence.getHealthHistory(query.hours || 24, query.limit || 100);
  }
  ...
  async getHealthSummary() {
    const [currentHealth, errorStats, unresolvedErrors, services] = await Promise.all([
      this.health.check([
        () => this.prismaIndicator.pingCheck('database', { timeout: 3000 }),
        () => this.memoryIndicator.checkHeap('memory_heap', 512 * 1024 * 1024),
      ]),
    ...
    const degradedServices = services.filter(
      (s) => s.status === ServiceStatus.DEGRADED || s.status === ServiceStatus.DOWN,
    );
