...
export type EmployeeAdminRecord = EmployeeWorkspaceRecord & {
  salary?: number;
  startDate?: string;
};
...
export function getEmployeeFormDefaults(
  ...
  return {
    ...
    password: "",
    role:
      (employee?.role as EmployeeAssignableRole | undefined) ?? UserRole.TEAM,
...
export function getInitialEmployeeAdminRecordById(employeeId: string) {
  return createInitialEmployeeAdminRecords().find(
