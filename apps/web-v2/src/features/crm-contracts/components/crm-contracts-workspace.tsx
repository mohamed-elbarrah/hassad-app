...
import { useGetCrmContractsIndexQuery } from "@/lib/api/crm-contracts-api";
import { useAppSelector } from "@/lib/store";

export function CrmContractsWorkspace() {
  ...
  const { data, error, isError, isLoading, refetch } = useGetCrmContractsIndexQuery(
    {
      status: statusParam,
      expiringDays:
        ...
        <CardContent>
          {authStatus !== "authenticated" || (isLoading && !data) ? (
            <WorkspaceQueryState
              ...
              loadingDescription="Retrieving the contract register from the CRM API."
            />
          ) : isError && !data ? (
            <WorkspaceQueryState
              kind="error"
