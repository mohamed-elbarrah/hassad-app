...
export function CrmChatWorkspace({
  ...
  } = useGetCrmChatConversationsQuery({ type: "DIRECT" });
  ...
  } = useGetCrmChatMessagesQuery(activeConversationId ?? skipToken);
  ...
  const { data: employeeTargets = [] } = useSearchCrmEmployeeChatTargetsQuery(searchArgs);
  const { data: clientTargets = [] } = useSearchCrmClientChatTargetsQuery(searchArgs);

  const [sendConversationMessage, sendConversationState] =
    useSendCrmConversationMessageMutation();
  const [sendDirectMessage, sendDirectState] = useSendCrmDirectMessageMutation();
