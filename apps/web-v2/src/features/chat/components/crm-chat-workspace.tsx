...
export function CrmChatWorkspace({ currentUserId }: CrmChatWorkspaceProps) {
  ...
  } = useGetCrmChatConversationsQuery({ type: conversationType });
  ...
  const { data: employeeTargets = [] } =
    useSearchCrmEmployeeChatTargetsQuery(searchArgs);
  const { data: clientTargets = [] } =
    useSearchCrmClientChatTargetsQuery(searchArgs);
