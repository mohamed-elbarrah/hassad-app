...
export function PmChatWorkspace({ currentUserId }: PmChatWorkspaceProps) {
  ...
  } = useGetPmChatConversationsQuery({ type: conversationType });
  ...
  const { data: employeeTargets = [] } =
    useSearchPmEmployeeChatTargetsQuery(searchArgs);
  const { data: clientTargets = [] } =
    useSearchPmClientChatTargetsQuery(searchArgs);
