...
export function TeamChatWorkspace({ currentUserId }: TeamChatWorkspaceProps) {
  ...
  } = useGetTeamChatConversationsQuery({ type: conversationType });
  ...
  const { data: employeeTargets = [] } =
    useSearchTeamEmployeeChatTargetsQuery(searchArgs);
  const { data: clientTargets = [] } =
    useSearchTeamClientChatTargetsQuery(searchArgs);
