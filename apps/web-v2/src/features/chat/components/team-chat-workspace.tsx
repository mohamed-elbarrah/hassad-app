...
export function TeamChatWorkspace({
  ...
  } = useGetTeamChatConversationsQuery({ type: "DIRECT" });
  ...
  } = useGetTeamChatMessagesQuery(activeConversationId ?? skipToken);
  ...
  const { data: employeeTargets = [] } = useSearchTeamEmployeeChatTargetsQuery(searchArgs);
  const { data: clientTargets = [] } = useSearchTeamClientChatTargetsQuery(searchArgs);

  const [sendConversationMessage, sendConversationState] =
    useSendTeamConversationMessageMutation();
  const [sendDirectMessage, sendDirectState] = useSendTeamDirectMessageMutation();
