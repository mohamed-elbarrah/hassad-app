...
export function AdminChatWorkspace({
  ...
  } = useGetChatConversationsQuery({ type: "DIRECT" });
  ...
  } = useGetChatMessagesQuery(activeConversationId ?? skipToken);
  ...
  const { data: employeeTargets = [] } = useSearchEmployeeChatTargetsQuery(searchArgs);
  const { data: clientTargets = [] } = useSearchClientChatTargetsQuery(searchArgs);

  const [sendConversationMessage, sendConversationState] =
    useSendConversationMessageMutation();
  const [sendDirectMessage, sendDirectState] = useSendDirectMessageMutation();
