...
export function AdminChatWorkspace({ currentUserId }: AdminChatWorkspaceProps) {
  ...
  } = useGetAdminChatConversationsQuery({ type: conversationType });
  ...
  const { data: employeeTargets = [] } =
    useSearchAdminEmployeeChatTargetsQuery(searchArgs);
  const { data: clientTargets = [] } =
    useSearchAdminClientChatTargetsQuery(searchArgs);
