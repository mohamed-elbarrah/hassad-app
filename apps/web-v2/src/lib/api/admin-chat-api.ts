...
export const adminChatApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getAdminChatConversations: builder.query<
    ...
    getAdminChatMessages: builder.query<ChatMessageRecord[], string>({
      query: (conversationId) => ({ url: `/admin/chat/conversations/${conversationId}/messages` }),
      transformResponse: (response: RawMessage[]) => response.map(mapMessage),
      providesTags: ["AdminChat"],
    }),
    searchAdminEmployeeChatTargets: builder.query<
      ChatTargetOption[],
      { search: string; limit?: number }
    >({
      query: ({ search, limit = 6 }) => ({
        url: "/admin/chat/targets/employees",
        params: {
          search,
          limit,
        },
    ...
    searchAdminClientChatTargets: builder.query<
      ChatTargetOption[],
      { search: string; limit?: number }
    >({
      query: ({ search, limit = 6 }) => ({
        url: "/admin/chat/targets/clients",
        params: {
          search,
          limit,
        },
    ...
    sendAdminConversationMessage: builder.mutation<
      ChatMessageRecord,
      {
        conversationId: string;
        content: string;
    ...
    >({
      query: ({ conversationId, content, parentMessageId, files = [] }) => {
        if (files.length > 0) {
          return {
            url: `/admin/chat/conversations/${conversationId}/messages/with-files`,
            method: "POST",
      ...
      invalidatesTags: ["AdminChat"],
    }),
    sendAdminDirectMessage: builder.mutation<
      ChatMessageRecord,
      {
        userId: string;
        content: string;
    ...
    >({
      query: ({ userId, content, parentMessageId, files = [] }) => {
        if (files.length > 0) {
          return {
            url: `/admin/chat/conversations/direct/${userId}/messages/with-files`,
            method: "POST",
            body: buildMessageFormData({ content, parentMessageId, files }),
          };
        }
        ...
        return {
          url: `/admin/chat/conversations/direct/${userId}/messages`,
          method: "POST",
          body: { content, parentMessageId },
        };
      },
    ...
    updateAdminChatMessage: builder.mutation<
      ChatMessageRecord,
      {
        conversationId: string;
        messageId: string;
    ...
    >({
      query: ({ conversationId, messageId, content }) => ({
        url: `/admin/chat/conversations/${conversationId}/messages/${messageId}`,
        method: "PATCH",
        body: { content },
      }),
      invalidatesTags: ["AdminChat"],
    }),
    deleteAdminChatMessage: builder.mutation<
      ChatMessageRecord,
      {
        conversationId: string;
        messageId: string;
