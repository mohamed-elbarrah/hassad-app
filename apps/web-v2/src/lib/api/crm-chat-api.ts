...
export const crmChatApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getCrmChatConversations: builder.query<
      { data: ChatConversationRecord[]; total: number; page: number; limit: number },
      { type?: "DIRECT" | "GROUP" | "PROJECT" }
    >({
      query: (params) => ({ url: "/crm/chat/conversations", params }),
      ...
      providesTags: ["CrmChat"],
    }),
    getCrmChatMessages: builder.query<ChatMessageRecord[], string>({
      query: (conversationId) => ({ url: `/crm/chat/conversations/${conversationId}/messages` }),
      transformResponse: (response: RawMessage[]) => response.map(mapMessage),
      providesTags: ["CrmChat"],
    }),
    searchCrmEmployeeChatTargets: builder.query<
      ChatTargetOption[],
      { search: string; limit?: number }
    >({
      query: ({ search, limit = 6 }) => ({
        url: "/crm/chat/targets/employees",
        params: {
          search,
          limit,
        },
    ...
    searchCrmClientChatTargets: builder.query<
      ChatTargetOption[],
      { search: string; limit?: number }
    >({
      query: ({ search, limit = 6 }) => ({
        url: "/crm/chat/targets/clients",
        params: {
          search,
          limit,
        },
    ...
    sendCrmConversationMessage: builder.mutation<
      ChatMessageRecord,
      {
        conversationId: string;
        content: string;
    ...
    >({
      query: ({ conversationId, content, parentMessageId, files = [] }) => {
        if (files.length > 0) {
          return {
            url: `/crm/chat/conversations/${conversationId}/messages/with-files`,
            method: "POST",
        ...
        return {
          url: `/crm/chat/conversations/${conversationId}/messages`,
          method: "POST",
          body: { content, parentMessageId },
        };
      },
      invalidatesTags: ["CrmChat"],
    }),
    sendCrmDirectMessage: builder.mutation<
      ChatMessageRecord,
      {
        userId: string;
        content: string;
    ...
    >({
      query: ({ userId, content, parentMessageId, files = [] }) => {
        if (files.length > 0) {
          return {
            url: `/crm/chat/conversations/direct/${userId}/messages/with-files`,
            method: "POST",
            body: buildMessageFormData({ content, parentMessageId, files }),
          };
        }
        ...
        return {
          url: `/crm/chat/conversations/direct/${userId}/messages`,
          method: "POST",
          body: { content, parentMessageId },
        };
      },
    ...
    updateCrmChatMessage: builder.mutation<
      ChatMessageRecord,
      {
        conversationId: string;
        messageId: string;
    ...
    >({
      query: ({ conversationId, messageId, content }) => ({
        url: `/crm/chat/conversations/${conversationId}/messages/${messageId}`,
        method: "PATCH",
        body: { content },
      }),
      invalidatesTags: ["CrmChat"],
    }),
    deleteCrmChatMessage: builder.mutation<
      ChatMessageRecord,
      {
        conversationId: string;
        messageId: string;
...
export const {
  useDeleteCrmChatMessageMutation,
  useGetCrmChatConversationsQuery,
  useGetCrmChatMessagesQuery,
  useSearchCrmClientChatTargetsQuery,
  useSearchCrmEmployeeChatTargetsQuery,
  useSendCrmConversationMessageMutation,
  useSendCrmDirectMessageMutation,
  useUpdateCrmChatMessageMutation,
} = crmChatApi;
