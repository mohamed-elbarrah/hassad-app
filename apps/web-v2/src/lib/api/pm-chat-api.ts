...
export const pmChatApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getPmChatConversations: builder.query<
      { data: ChatConversationRecord[]; total: number; page: number; limit: number },
      { type?: "DIRECT" | "GROUP" | "PROJECT" }
    >({
      query: (params) => ({ url: "/pm/chat/conversations", params }),
    ...
    >({
      query: ({ conversationId, content, parentMessageId, files = [] }) => {
        if (files.length > 0) {
          return {
            url: `/pm/chat/conversations/${conversationId}/messages/with-files`,
            method: "POST",
        ...
        return {
          url: `/pm/chat/conversations/${conversationId}/messages`,
          method: "POST",
    ...
    >({
      query: ({ userId, content, parentMessageId, files = [] }) => {
        if (files.length > 0) {
          return {
            url: `/pm/chat/conversations/direct/${userId}/messages/with-files`,
            method: "POST",
            body: buildMessageFormData({ content, parentMessageId, files }),
          };
        }
        ...
        return {
          url: `/pm/chat/conversations/direct/${userId}/messages`,
          method: "POST",
          body: { content, parentMessageId },
        };
      },
