...
export interface NormalizedApiError { status: number; message: string; details?: unknown; }
...
export function handleMutationError(err: unknown, fallback: string, toast?: typeof import("sonner").toast): void;  // toast.error(normalizeApiError(err).message || fallback)
```
...
```ts
export function usePortalDownload(scope: "invoice" | "strategy" | "periodReport" | "periodFile")
  : { download: (id: string, extra?: { periodId?: string; fileId?: string }) => Promise<void>; isDownloading: boolean; };
```
...
<QueueToolbar
  ...
  filterGroups={filterGroups} activeFilters={activeFilters} onFilterChange={handleFilterChange}
  countLabel="حملة" count={total}
/>
```
...
```tsx
<DetailBreadcrumb backHref={PORTAL_ROUTES.contracts} backLabel="العقود" title={data.title} />
```
